...
const SearchFlightsForm = (props) => {
  const { DescGetter, flightsData } = props
    ...
    switch (type) {
      case 'adults':
        mode === '+'
          ...
          : setAdultCount(adultCount ? adultCount - 1 : 0)
        break
      case 'children':
        mode === '+'
          ? setChildCount(childCount + 1)
          : setChildCount(childCount ? childCount - 1 : 0)
        break
      case 'infants':
        mode === '+'
          ? setInfantCount(infantCount + 1)
          : setInfantCount(infantCount ? infantCount - 1 : 0)
        break
      default:
        return
    }
  ...
  const handleChange = (e) => {
    ...
    if (type === 'from-city') {
      const toObject = fromObjects.find((i) => i.code === value)
  ...
  const submittable =
    Object.keys(fromRoute).length && adultCount + childCount + infantCount > 0
  ...
  return (
    <form onSubmit={handleSubmit} className="search-box">
      ...
      <label className="search-label">Search For Flights:</label>
      ...
      <div className="d-flex w-100p inputs-container">
        <div className="d-flex column flex-one">
          ...
          >
            {fromObjects?.map((i) => (
              <option key={i.code} value={i.code} readOnly>
                {i.name}
              </option>
            ))}
          </select>
      ...
      <div className="d-flex w-100p seat-counters">
        <div className="d-flex row">
          <label className="adults-label">Adults:</label>
          <span>
            <button
        ...
        <div className="d-flex row">
          <label className="children-label">Children:</label>
          <span>
            <button
        ...
        <div className="d-flex row">
          <label className="infants-label">Infants:</label>
          <span>
            <button
